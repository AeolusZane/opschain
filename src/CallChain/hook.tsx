...
import { Typography } from "@fd/react";
const { Text } = Typography;
export const formatStringListToOptions = (id: string) => {
  return {
    value: id,
    label: id,
  };
...
export function useSetDateTimeRange() {
  ...
  const DefaultPickerValue: RangeValue = [
    dayjs(+new Date() - 30 * 60 * 1000),
    dayjs(+new Date()),
  ];
  ...
  const disabledDate = (current: Dayjs) => {
    ...
    const tooLate =
      dates[0] && current.diff(dates[0], "days") >= (disableRange || 3);
    const tooEarly =
      dates[1] && dates[1].diff(current, "days") >= (disableRange || 3);
...
export type SelectorHookType = [
  DebouncedFunc<React.Dispatch<React.SetStateAction<string>>>,
  SelectProps["options"]
];
 ...
 * 访问内容
 */
export function useSceneTypeSelector(): SelectorHookType {
  const { data, setSceneType } = useQuerySceneType();
  /**
   * 下拉的options列表
   */
   ...
   */
  const setSearchSceneType = debounce(setSceneType, 400);

  useEffect(() => {
    const noChoose = {
      value: "all",
      label: <Text type={"secondary"}>全部</Text>,
    };
    const list = data?.map(formatStringListToOptions);
    setSceneTypeList(list ? [noChoose, ...list] : [noChoose]);
  }, [data]);

  return [setSearchSceneType, sceneTypeList];
}
...
export function useQuerySceneTypeItemSelector(): SelectorHookType {
   ...
   * 下拉的options列表
   */
  const [sceneTypeItemList, setSceneTypeItemList] = useState<
    SelectProps["options"]
  >([]);
  /**
   * 根据当前输入的sceneType做模糊搜索，刷新options
...
export function useScenceTagKeySelector(): SelectorHookType {
   ...
   * 下拉的options列表
   */
  const [traceTagKeyList, setTraceTagKeyList] = useState<
    SelectProps["options"]
  >([]);
  /**
